...
async function getLatestDbThreadIdByQueueKey(queueKey) {
  const pgClient = new Client(clientConfig);
  await pgClient.connect();
  const res = await pgClient.query('SELECT thread_id FROM lfg_queues WHERE queue_key = $1 ORDER BY updated_at DESC LIMIT 1', [queueKey]);
  await pgClient.end();
  return res.rows[0]?.thread_id || null;
}
...
async function updateStarterMessage(thread, queueDef, members) {
  const starter = await thread.fetchStarterMessage().catch(() => null);
  if (!starter) return;
  const img = await generateQueueImage(thread.client, queueDef, members);
  await starter.edit({ files: [img], embeds: [buildEmbed(queueDef, members, img.name)], components: [buildButtons(queueDef.key)] });
}

async function handleJoin(interaction, key) {
  const q = QUEUES.find(x => x.key === key);
  if (!q) return interaction.reply({ flags: 64, content: 'Queue not found.' });
  const members = ensureQueueState(key);
  members.add(interaction.user.id);
  const thread = interaction.channel;
  await updateStarterMessage(thread, q, members);
  await interaction.reply({ flags: 64, content: `Joined ${q.name}.` });
  if (members.size >= q.size) {
    const picks = [...members].slice(0, q.size);
    picks.forEach(id => members.delete(id));
    const mention = picks.map(id => `<@${id}>`).join(' ');
...
async function handleLeave(interaction, key) {
  ...
  const thread = interaction.channel;
  await updateStarterMessage(thread, q, members);
  await interaction.reply({ flags: 64, content: `Left ${q.name}.` });
}

async function handleStatus(interaction, key) {
  const q = QUEUES.find(x => x.key === key);
  if (!q) return interaction.reply({ flags: 64, content: 'Queue not found.' });
  const members = ensureQueueState(key);
  await interaction.reply({ flags: 64, content: `${members.size}/${q.size} waiting in ${q.name}.` });
}
