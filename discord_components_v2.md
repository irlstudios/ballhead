...
## 3. Modal Components (Dropdowns Inside Modals)

Modal components now support dropdowns and other select menus when you use labels.  
Make sure your versions are new enough:
- `discord.js` v14.23.0+
- `@discordjs/builders` v1.12.0+

Key rules:
- Modals must use `LabelBuilder` components.
- Each label must include a component (select or text input).
- Action rows inside modals are deprecated.

### Show a modal with a dropdown + text input
```js
const {
  ModalBuilder,
  LabelBuilder,
...
const selectMenu = new StringSelectMenuBuilder()
  .setCustomId('favorite_snack')
  .setPlaceholder('Choose your favorite snack')
  .setMinValues(1)
  .setMaxValues(1)
  .addOptions(
    new StringSelectMenuOptionBuilder()
      .setLabel('Apple')
      .setDescription('A very healthy option.')
      .setEmoji('🍎')
      .setValue('Red Apple'),
    new StringSelectMenuOptionBuilder()
...
const modal = new ModalBuilder()
  .setCustomId('snack_modal')
  .setTitle('Select Your Snack')
  .addLabelComponents(
    new LabelBuilder()
      .setLabel('Favorite Snack')
      .setDescription('Choose your favorite snack from the list.')
      .setStringSelectMenuComponent(selectMenu),
    new LabelBuilder()
      .setLabel('Reason')
      .setDescription('Tell us why you picked it.')
...
### Read the modal submission
```js
const snack = interaction.fields.getStringSelectValues('favorite_snack')[0];
const reason = interaction.fields.getTextInputValue('reason_input');
```

---

## 4. Full Slash Command Example
